            ...
            async move {
                ...
                let event_collector = tokio::time::timeout(
                    ...
                    crate::meetup::sync::sync_task(meetup_client.clone(), &db_connection).map_err(
                        |err| {
                            error!("Syncing task failed: {}", err);
                            err
                        },
                    ),
                )
                .map_err(|err| {
                    error!("Syncing task timed out: {}", err);
                    err
                })
                .await??;
                // Sync with Discord
                ...
                .await
                {
                    error!("Discord syncing task failed: {}", err);
                }
                ...
                if let Some(channel_id) = crate::discord::sync::ids::FREE_SPOTS_CHANNEL_ID {
                    if let Err(err) = event_collector
                        ...
                        .await
                    {
                        error!("Error when posting open game spots:\n{:#?}", err);
                    }
                } else {
                    error!("No channel configured for posting open game spots");
                }
                if let Err(err) = event_collector
                    .assign_roles(meetup_client.clone(), &db_connection, &discord_api)
                    .await
                {
                    error!("Error in EventCollector::assign_roles:\n{:#?}", err);
                }
                Ok::<_, crate::BoxedError>(())
            }
            .instrument(info_span!("create_recurring_syncing_task")),
        );
    }
}
