...
> {
    ...
    if let (&Method::GET, "/authorize") = (method, path) {
        ...
        let (redis_connection, csrf_cookie) =
        ...
        Response::builder()
            ...
            .map(|response| {
                (
                    redis_connection,
                    Some(super::server::HandlerResponse::Response(response)),
                )
            })
        ...
        if let Some(error) = error {
            return Ok((
                redis_connection,
                Some(("OAuth2 error", error.to_string()).into()),
            ));
        }
        let (code, csrf_state) = match (code, state) {
            ...
            _ => {
                return Ok((
                    redis_connection,
                    Some(("Request parameters missing", "").into()),
                ))
            }
        };
        ...
        if !is_organizer {
            return Ok((
                redis_connection,
                Some(("Only the organizer can log in", "").into()),
            ));
        }
        ...
        let (redis_connection, _): (_, ()) = lib::redis::async_redis_transaction(
            redis_connection,
        ...
        *async_meetup_client.lock().await = Some(Arc::new(new_async_meetup_client));
        Ok((
            redis_connection,
            Some(("Thanks for logging in :)", "").into()),
        ))
        ...
        (method, lib::urls::LINK_URL_REGEX.captures(path))
    {
        ...
        let linking_id = match captures.name("id") {
            Some(id) => id.as_str(),
            _ => return Ok((redis_connection, Some(("Invalid Request", "").into()))),
        };
        ...
        (method, lib::urls::LINK_REDIRECT_URL_REGEX.captures(path))
    {
        ...
        let meetup_user = match meetup_user {
            ...
            _ => {
                return Ok((
                    redis_connection,
                    Some(("Could not find Meetup ID", "").into()),
                ));
            }
        };
                    ...
                    return Ok((
                        ...
                        Some(
                            (
                                ...
                                lib::strings::OAUTH2_DISCORD_ALREADY_LINKED_FAILURE_CONTENT(
                                    &bot_name,
                                ),
                            )
                                .into(),
        ...
        if let Some(photo) = meetup_user.photo {
            Ok((
                redis_connection,
                super::server::HandlerResponse::Message {
